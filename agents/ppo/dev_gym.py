...
WoodDestoryReward = 1
OreDestoryReward = 3
EnemyDestoryReward = 1

def in_flame(bx, by, ex, ey, dist):
    if (abs(bx - ex) <= dist):
        return True
    if (abs(by - ey) <= dist):
        return True
...
def calculate_reward(state: Dict, last_state: Dict, action, unit_id):
    target_unit = state.unit_state.unit_id
    hp_reward = target_unit.hp - last_state.unit_state.unit_id.hp
    tick_reward = -0.1 * state.tick
    # Get Destory Reward
    destory_reward = 0
    for entity in state.entities:
        if (entity.type == "w" and in_flame()):
            destory_reward += WoodDestoryReward
        elif (entity.type == "o"):
            destory_reward += OreDestoryReward
    # Get EnemyReward
    destroy_reward = 1 * 
    return 1
